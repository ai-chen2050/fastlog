...
pub fn sign_message_with_chainid(
    secret: [u8; 32],
    raw_message: &str,
    chain_id: u64,
) -> anyhow::Result<(Alloy_Signature, String), anyhow::Error> {
    let secret = B256::new(secret);
    let signer =
        PrivateKeySigner::from_bytes(&secret).map_err(|_| secp256k1::Error::InvalidSecretKey)?;
...
pub fn recover_signer_alloy(sig: String, raw_msg: &str) -> anyhow::Result<Address, anyhow::Error> {
...
mod tests {
    ...
    fn recover_address() {
        ...
        let recover_sig = Signature::_from_hex_str(sig_hex);
