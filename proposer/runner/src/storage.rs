...
#[derive(Default)]
pub struct Storage {
    pub _pg_db: Arc<DatabaseConnection>,
}

impl Storage {
    pub async fn new(config: Arc<ProposerConfig>) -> Self {
        // connect to pg db
    ...
    // postgre inner api
    pub async fn _sinker_clock(&self, message_id: String, raw_message: Vec<u8>) {
    ...
    pub async fn _get_clocks_counts(&self) -> Result<u64, DbErr> {
        let clocks_count = ClockInfos::find().count(self._pg_db.as_ref()).await;
